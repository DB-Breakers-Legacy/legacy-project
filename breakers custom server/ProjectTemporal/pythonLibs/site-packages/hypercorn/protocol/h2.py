...
async def getRandomBytes():
    return await asyncio.get_event_loop().run_in_executor(None, os.urandom, 8)
#
...
class H2Protocol:
    ...
    async def _send_data(self, stream_id: int) -> None:
        try:
            ...
            if data:
                self.connection.send_data(stream_id, data) #BREADED - todo: check if large data is fine Update: no its not
                await self._flush()
            else:
